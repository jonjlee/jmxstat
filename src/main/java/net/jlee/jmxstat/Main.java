...
public class Main {
    ...
    private static void usage() {
        error("Usage:\n" //
                ...
                + "[mbean.name[attribute.field], ...] [interval]", 1);
    }

    // Parse the list of mbean attributes from the command line (arguments 2 and
    // on)
    private static List<MbeanAttr> parseMbeans(String[] args)
        ...
        for (int i = 1; i < args.length; i++) {
            ...
            if (startIdx >= 0 && endIdx > startIdx) {
                ObjectName objectName = new ObjectName(def.substring(0,
                        startIdx));
                String[] attrs = def.substring(startIdx + 1, endIdx).split(",");
                for (String attr : attrs) {
                    int subIdx = attr.indexOf(".");
                    if (subIdx >= 0) {
                        attrList.add(new MbeanAttr(objectName, attr.substring(
